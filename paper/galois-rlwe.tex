...
& = (f \ast g)(a).
\end{align*}
The general case will follow from induction on $n$.
\end{proof}
...
Taking $f = e_\alpha$ and $e_\alpha'$, we immediately obtain
...
The punchline is: the value $\epsilon'(m,q,k)$ is usually negligibly small. Since $\epsilon'(m,q,k)$ is an upper bound of  the distribution $P_{m,k} \pmod {\fq}$ is computationally indistinguishable from the uniform distribution for all $\fq$. The following is a table of data to illustrate this phenomenon.
...
The data in Table~\ref{tab: deg1} suggests that for $n \geq 100$ and $q = O(n)$, the statistcal distance between $P'_{m,k} \pmod{\fq}$ and the uniform distribution is negligibly small. As a consequence, any attack that reduce the errors modulo $\fq$ will be unlikely to succeed.
...
\]
Since the sums in the definition of $d(a)$ converge rapidly, we could obtain good approximations of the values of $d$ by truncating the sums. Next, we compute its Fourier transform $\hat{d}$, and obtain
\[
...
Then $\epsilon(m,q,\tau)$ is an upper bound of the statistical distance  between the distribution $e_\alpha$ and the uniform distribution over $\bF_q$. Table~\ref{table: PLWE} contains some data.
