...
let $T$ denote the unitary matrix \Katetodo{I added the word orthonormal; that's correct, right?  I want to make sure the error distribution is spherical.}
\[
T = \begin{bmatrix}
    I_{r_1}  & 0  \\
    0     & T_{r_2} \\
...
To decide whether the samples are from a uniform distribution, we can either look up a table of $\chi^2$ values, or use an approximation rule:  when $d$ is large, the $\chi^2$ distribution can be well-approximated by a normal distribution $N(d, 2d)$. More precisely, we set a confidence level parameter $\alpha \in (0,1)$, and compute $\delta = \delta(\alpha) > 0$ such that $\prob( \chi^2_{r-1} \in (d-\delta, d+\delta)) = \alpha$. Finally, we reject the hypothesis that the samples $S$ are drawn from the uniform distribution if and only if  $|\chi^2(S,y) -d | \geq \delta$.
...
\Katetodo{I think we need to discuss confidence level in the background here}
\Katetodo{I think we need to be specific on how to test the output of the chi-square statistic in terms of confidence level; we use this formula in the Algorithm 1}

\subsection{The chi-square attack on $\SRLWE(\cR,\fq)$}

Let $\cR$ be an RLWE instance with error distribution $D_{\cR}$ and $\fq$ be a prime ideal above $q$.  The basic idea of our attack relies on the assumption that the distribution $D_\cR \pmod {\fq}$ is distinguishable from the uniform distribution on the finite field $F = R/\fq$. More precisely, the attack loops through all $q^f$ possitbilities of $\bar{s} = s \pmod{\fq}$, and for each guess $s'$, it computes the values $\bar{e}' = \bar{b} - \bar{a} s' \pmod {\fq}$ for every sample $(a,b) \in S$. If the guess is wrong, or if the samples are taken from the uniform distirbution in $(R_q)^2$, the values $\bar{e}'$ would be uniformly distributed in $F$ and it is likely to pass the chi-square test. On the other hand, if the guess is correct, then we expect the test on the errors $\bar{e}'$ to reject the null hypothesis. We remark that one needs at least $\Omega(N)$ samples for the test to work effectively. 

Let $N = q^f$ denote the cardinality of $F$. For the attack to be successful, we need the $(N-1)$ tests corresponding to wrong guess of $s \pmod{\fq}$ to pass, and the one test corresponding to the correct guess to be rejected. To this purpose, we need to choose the confidence level $\alpha$ to be close enough to one. (An example choice of $\alpha$ is $1 - \frac{1}{10N}$). The detailed attack is described in Algorithm~\ref{alg: chi-square}.  Let $\Phi$ denote the cumulative distribution function of 
the standard normal distirbution.
...
%Note that although we restrict ourselves to subfields of cyclotomics with odd and square-free $m$, the attack could be applied to any finite extension of $\bQ$.
    \Katetodo{In alg: Explain what $\Phi$ is in earlier background matter}
    \Katetodo{In alg: Why is is 1/20N instead of 1/10N as discussed above?  Should we let the confidence level be part of the input (Require)?}
    \Katetodo{We need to put F into the Require}
    ...
    \Katetodo{I think step 9 could be written out and assigned to a variable, it's just a formula}
    \Katetodo{Is elsereturn a word?}
...
\begin{algorithmic}[1]              % enter the algorithmic environment
     ...
     $\cR = (K,q,\sigma, s)$ -- an RLWE instance; $R$ -- the ring of integers of $K$; $\fq$ -- a prime ideal in $K$ above $q$; $F = R/\fq$ -- the residual field of $\fq$; $N$ -- the cardinality of $F$; $\mathcal{S}$ -- a collection of $M$ ($M = \Omega(N)$) RLWE samples from $\cR$; $0 < \alpha < 1$ -- the confidence level. 
    \Ensure a guess of the value $s \pmod{\fq}$, or {\bf NOT-RLWE}, or {\bf INSUFFICIENT-SAMPLES}

    \State $\delta \gets \Phi^{-1}(\frac{1+\alpha}{2})$, $\cG \gets \emptyset$.
    \For{$s$ in $F$}
        ...
        \If{$|\chi^2(\cE) - (N-1)| > \delta \sqrt{2N-2}$}
            \State add $s$ to $\cG$.
        \EndIf
    \EndFor
    \If{$G = \emptyset$}
...
The time complexity of the attack is $O(N)$ since there are $N$ possible values for $s \pmod {\fq}$. The number of samples need for the attack is also $O(N)$. The correctness of the attack is captured in Theorem~\ref{thm: attack} below. We use $D_{\cR,\fq}$ as a shorthand notation for $D_{\cR} \pmod{\fq}$. \Katetodo{The explanation of $\Phi$ should go in background along with confidence level stuff.}
\Katetodo{This requirement of the chi-square test should go in background section}


\begin{theorem} \label{thm: attack}
Let $\Delta$ denote the statistical distance between the distribution $D_{\cR, \fq}$ and the uniform distribution on $R/\fq$. Let $\lambda = 4 M \Delta^2$ and let $\delta = \Phi^{-1}(\frac{1+\alpha}{2})$. Finally, let $p$ denote the probability of success of the attack in Algorithm~\ref{alg: chi-square}. When $q \gg 1$, we have

$$p  \geq \alpha^{N-1} \left(1- \Phi \left(\frac{\delta \sqrt{2(N-1)}- \lambda}{\sqrt{2(N-1) +4\lambda}}\right) \right) - o(1).$$
\end{theorem}
\Katetodo{It would be nice to state this theorem in terms of a general confidence level instead of the specific one 1-1/20N}

\begin{proof}
It is a standard fact that chi-square value on samples from $D_{\cR, \fq}$  follows a noncentral chi-square distribution with the same degree of freedom and a parameter $\lambda_0$ given by
\[
    \lambda_0 =  d_2(D_{\cR, \fq}, U(R/\fq))^2 \cdot MN.
\]
In particular, we have $\lambda_0 \geq  4M\Delta^2 = \lambda$. Since $q \gg 1$, we may approximate a noncentral chi-square distribution with degree of freedom $k$ and parameter $\lambda_0$ with a Gaussian distribution of mean $k + \lambda_0$ and variance $2k + 4\lambda_0$ (see \cite{ryabko2004new}, for example). Recall that our attack succeeds if the set $\cE$ from all $(N-1)$ wrong guesses passes the test, and the true errors modulo $\fq$ fail the test. Assuming these samples are independent of each other, the first event happens with probability $\alpha ^{N-1}$, whereas the second event has probability $1 - \Phi \left(\frac{\delta \sqrt{2(N-1)}- \lambda_0}{\sqrt{2(N-1) +4\lambda_0}}\right) $, which is an increasing function in $\lambda_0$. The theorem now follows. The $o(1)$ term reflects the approximation error when we replaced the chi-square distributions in question by Gaussians.
\end{proof}
...
\]
\Katetodo{I'm a little worried about this.  is this true?  In any case it's not the adjusted embedding, but more than that where are you using C?}
