...
\begin{algorithmic}[1]              % enter the algorithmic environment
     ...
     $\cR = (K,q,\sigma, s)$ -- an RLWE instance; $R$ -- the ring of integers of $K$; $\fq$ -- a prime ideal in $K$ above $q$; $F = R/\fq$ -- the residual field of $\fq$; $\mathcal{S}$ -- a collection of $M$ ($M = \Omega(|F|)$) RLWE samples from $\cR$; $0 < \alpha < 1$ -- the confidence level. 
...
The time complexity of the attack is $O(N)$ since there are $N$ possible values for $s \pmod {\fq}$. The number of samples need for the attack is also $O(N)$.
\Katetodo{This requirement of the chi-square test should go in background section}

The correctness of the attack is captured in Theorem~\ref{thm: attack}. We use $D_{\cR,\fq}$ as a shorthand notation for $D_{\cR} \pmod{\fq}$. \Katetodo{The explanation of $\Phi$ should go in background along with confidence level stuff.}

\begin{theorem} \label{thm: attack}
Let $\Delta$ denote the statistical distance between the distribution $D_{\cR, \fq}$ and the uniform distribution on $R/\fq$. Let $\lambda = 4 M \Delta^2$ and $\omega = \Phi^{-1}(1- \frac{1}{20N})$. Finally, let $p$ denote the probability of success of the attack in Algorithm~\ref{alg: chi-square}. When $q \gg 1$, we have
