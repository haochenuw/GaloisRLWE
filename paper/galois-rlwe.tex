...
{\bf Notation}: for each subgroup $H$ of $G = (\bZ/m\bZ)^*$, we use $K_{m,H}$ to denote the fixed field of $K_{m,H} = \bQ(\zeta_m)^H$.
...
In order to illustrate our discussion above and the search-to-decision reduction (Theorem~\ref{}), we study an sub-cyclotomic RLWE instance in detail, where we generate RLWE samples, perform the attack, and used search-to-decision reduction to recover the entire secret key $s$.
