...
For a given $\sigma$, $k, \beta$, suppose $d(D_{\bZ, \sigma} \pmod{q}, V_{k,\alpha}) = \delta$, then $|\widehat{D} - \widehat{V}|_\infty  \leq  2\delta$.

\begin{Lemma} \label{lem: dist2}
\[
    d(e_\alpha, e'_{\alpha}) \leq  \sum_{y \in \bF_q, y \neq 0} \left( \prod_{i=0}^{n-1} \left(\cos(\frac{\pi y \alpha^i}{q})  + 2\delta \right) - \prod_{i=0}^{n-1} \cos(\frac{\pi y \alpha^i}{q}) \right).
\]
\end{Lemma}

Given $\sigma > 0$, one can find parameters $k, \beta$ so that $\delta$ is small, hence we could use the Lemma~\ref{lem: dist2} to bound the statistical distance between the two error distributions. As an example (fixme: add examples).
