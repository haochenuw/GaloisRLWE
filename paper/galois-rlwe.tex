...
\begin{abstract}
We describe a new attack on thte Ring learning-with-errors (RLWE) problem based on the chi-square statistical test, and give examples of Galois number fields vulnerable to our attack. We also analyze the security of cyclotomic fields under our attack using Fourier analysis on finite fields.
...
255255 &  [97943, 162436, 253826, 248711, 44318] & 90 & 2003 & 2 & 1.25 & 15000 &  1114.44 (est) \\
285285 & [181156, 210926, 87361] & 96 & 521  & 2 & 1.1 & 5000 & 75.41 (est) \\
...
\]
It is conceivable that this property would make $\pi(e)$ small in $\bF_q$, thus making the instance vulnerable to our attack.
...
f\section{Invulnerability of general cyclotomic extensions for unramified primes}
\label{sec: cyclo-secure}

\subsection{Introduction}

Let $m \geq 1$ be an integer and let $K = \bQ(\zeta_m)$ be the $m$-th cyclotomic filed. Let $q$ be a prime such that
$q \equiv 1\pmod{m}$, so $q$ is unramified in $K$. Finally, let $\fq$ be a prime ideal above $q$. We show that under some simplifying assumptions, the reduced RLWE error distribution $D_{\cR,\fq}$ will be indistinguishable from the uniform distribution $U(\bF_q)$. Note that this result is in contrast with the previous section, where we showed that the ramified prime for
...
\begin{Definition}
For an even integer $k \geq 2$ and $0 \leq \beta \leq 1$ let $\cV_{k,\beta}$ denote the distribution over $\bZ$ such that for every $t \in \bZ$,

$$\prob(\cV_{k,\beta} = t) =  \begin{cases} \beta^{t+\frac{k}{2}} (1-\beta)^{\frac{k}{2}-t} {k \choose t+\frac{k}{2}} &\mbox{if } |t| \leq \frac{k}{2} \\
0 & \mbox{otherwise}  \end{cases}$$

\end{Definition}
When $q > k$, we abuse notations and also use $\cV_{k,\beta}$ to denote the reduced reduced distribution $\cV_{k,\beta} \pmod {q}$ over $\bF_q$, and let $\nu_{k,\beta}$ denote its probability density function.

\begin{figure}[h!]
\centering
\includegraphics[width = 0.5\textwidth]{v8.png}
\caption{Probability density function of $\cV_{8,0.5}$}
\end{figure}
...
\[
    \sum_{i=0}^{n-1} e_i \zeta_m^{i} \pmod{qR},
\]
...
\[
    f(a) = \frac{1}{q} \sum_{y \in \bF_q} \widehat{f}(y)\chi_y(a).
\]
...
For all even integers $k \geq 2$, $\widehat{\cV_k}(y)  = \cos \left(\frac{\pi y}{q}\right)^k.$
\end{Lemma}

\begin{proof} The proof is simply unwinding all the definitions. We have
\begin{align*}
2^k \cdot \widehat{\cV_k}(y) &= \sum_{m = -\frac{k}{2}}^{\frac{k}{2}} {k \choose m+\frac{k}{2}} e^{2\pi i ym/q}  \\
...
Now we come back to the error distribution $MD(m,q,k) \pmod {\fq}$. There is a one-to-one correspondence between primitive $m$-th roots of unity in $\bF_q$ and the prime ideals above $q$ in $\bQ(\zeta_m)$. Let $\alpha$ be the root corresponding to our choice of $\fq$. Then a sample from $MD(m,q,k) \pmod {\fq}$ is
$$E_{\alpha} := \sum_{i=0}^{n-1} \alpha^i E_i \pmod {q},$$ where $E_i$ are independent variables under the distribution $\cV_k$. Let $e_\alpha$ denote the probability density function of $E$  The following lemma is crucial to our analysis:
...
Now we are able to bound the difference $e_\alpha(a) - 1/q$ using the Fourier inversion formula.

\begin{theorem}
For all $a \in \bF_q$,
\begin{equation} \label{eq: secure}
    |e_\alpha(a) -  1/q| \leq \frac{1}{q}  \sum_{y \in \bF_q, y \neq 0}  |\widehat{e_\alpha}(y)|.
\end{equation}
\end{theorem}

\begin{proof}
\begin{align*}
    e_\alpha(a) - 1/q &= e(a) - u(a) \\
    & = \frac{1}{q} \sum_{y \in \bF_q} (\widehat{e_\alpha}(y) - \widehat{u}(y) )\chi_y(a) \\
& = \frac{1}{q} \sum_{y \in \bF_q} (\widehat{e_\alpha}(y) - \delta(y) )\chi_y(a) \\
& = \frac{1}{q} \sum_{y \in \bF_q, y \neq 0} \widehat{e_\alpha}(y) \chi_y(a).  \qquad \mbox{(since $\widehat{e_\alpha}(0) = 1$)}
\end{align*}
Now the theorem follows from taking absolute values on both sides, and noting that $|\chi_y(a)| \leq 1$ for all $a$ and all $y$.
\end{proof}

\begin{Corollary} \label{cor: stat dist}
The statistical distance between $e_\alpha$ and $u$ satisfies $$d(e_\alpha,u) \leq \frac{1}{2}  \sum_{y \in \bF_q, y \neq 0}  |\widehat{e_\alpha}(y)|.$$
\end{Corollary}



Let $\epsilon(m,q,k,\alpha)$ denote the right hand side of (fixme), i.e.,
\[
...
The punchline of our argument is: the value $\epsilon(m,q,k)$ is usually negligibly small. Since $\epsilon(m,q,k)$ is an upper bound of  the distribution $MD(m,q,k) \pmod {\fq}$ is computationally indistinguishable from the uniform distribution for all $\fq$. The following is a table of data to illustrate this phenomenon.
...
We remark that we can run the same analysis for the PLWE distribution. The difference is that the coefficients for $\zeta_m^i$ are sampled from a discrete Gaussian distribution $D_{\bZ,\sigma}$, and there is no obvious closed-form formula for the density function $d$  of $D_{\bZ,\sigma} \pmod{q}$. Nonetheless, we could numerically approximate this probability density function, using the formula
\[
  d(a)  = \frac{\sum_{z \in \bZ, z \equiv a\pmod{q}} e^{-|z|^2/2\sigma}}{\sum_{z \in \bZ} e^{-|z|^2/2\sigma}}, \quad \forall a \in \bF_q.
\]
Since the sums in the definition of $d(a)$ converge rapidly, we could estimate $d(a)$ by truncating the sums. Next, we compute
its Fourier transform $\hat{d}$, and obtain
\[
    \widehat{e_\alpha'}(y) = \prod_{i=0}^{n-1} \hat{d}(\alpha^i y)
\]
Finally, we could define and compute
\[
    \epsilon'(m,q,\tau) = \frac{1}{2} \sum_{y \in \bF_q, y \neq 0} \prod_{i=0}^{n-1} \widehat{d}(\alpha^i y).
\]


Then an exact same argument as Corollary~\ref{cor: stat dist} says that $\epsilon'(m,q,\tau)$ is an upper bound of the statistical distance  between the distribution $e_\alpha'$ and the uniform distribution over $\bF_q$. Table~\ref{table: PLWE} contains some data.
...
\[
    \widehat{e_\alpha}(y) = \prod_{i=1}^{n} \cos \left(\frac{ \pi Tr(\alpha^i y) }{q} \right)^k.
\]
